...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, layout = 'grid' }) => {
    // Ưu tiên hình ảnh mock up (Home), nếu không có thì lấy đại diện
    const mainImage = project.images?.home || 'https://via.placeholder.com/600x400?text=No+Image';

    // Hàm tối ưu hóa URL Cloudinary
    const getOptimizedCloudinaryUrl = (url: string) => {
        if (!url) return url;
        // Kiểm tra xem có phải URL Cloudinary không và chưa có tham số f_auto,q_auto
        if (url.includes('cloudinary.com') && !url.includes('/upload/f_auto,q_auto/')) {
            // Chèn các tham số tối ưu hóa vào URL sau đoạn /upload/
    ...
    return (
        ...
        >
            <div className={`relative overflow-hidden bg-slate-800 ${layout === 'list' ? 'md:w-5/12 shrink-0 h-64 md:h-auto min-h-[250px]' : 'aspect-video'}`}>
                {/* Có thể tích hợp Skeleton ở đây khi ảnh thật đang load */}
                <img
                    src={optimizedImage}
                    alt={project.title}
                    className="object-cover w-full h-full group-hover:scale-110 transition-transform duration-500 absolute inset-0"
                    loading="lazy"
                />
